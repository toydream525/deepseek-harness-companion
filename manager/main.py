...
class MainWindow(QMainWindow):
    ...
    def _diagnostics_tab(self):
        ...
        diagnostic_actions.addWidget(button("导出脱敏诊断", self.export_diagnostics, "primaryButton"))
        diagnostic_actions.addStretch()
        panel_layout.addLayout(diagnostic_actions)
        layout.addWidget(panel)
    ...
    def _runtime_tab(self):
        ...
        chooser.addWidget(button("复制路径", lambda: self.copy_text(self.runtime_path_edit.text())))
    ...
    def _components_tab(self):
        ...
        dsh_row.addWidget(button("复制", lambda: self.copy_text(self.dsh_root_edit.text())))
        ...
        node_row.addWidget(button("复制", lambda: self.copy_text(self.node_path_edit.text())))
    ...
    def _overview_page(self):
        ...
        advanced_layout.setContentsMargins(28, 24, 28, 24)
        ...
        setup_actions = QGridLayout()
        setup_actions.setHorizontalSpacing(10)
        setup_actions.setVerticalSpacing(10)
        for label, handler in (("运行环境与 DSH", self.open_components_settings),
            ...
            index = setup_actions.count()
            setup_actions.addWidget(button(label, handler), index // 3, index % 3)
        ...
        control_row.addWidget(self.take_control_btn)
        control_row.addStretch()
        advanced_layout.addLayout(control_row)
        ...
        for widget in (self.dsh_start_btn, self.dsh_stop_btn):
            dsh_actions.addWidget(widget)
        dsh_actions.addStretch()
        dsh_layout.addLayout(dsh_actions)
        dsh_danger_row = QHBoxLayout()
        dsh_danger_row.addStretch()
    ...
    def _chat_page(self):
        ...
        self.chat_info.setObjectName("sectionHint")
        chat_top.addWidget(self.chat_info, 1)
        self.chat_settings_button = button("对话设置", self._toggle_chat_settings)
        chat_top.addWidget(self.chat_settings_button)
        outer.addLayout(chat_top)
        self.chat_settings_panel = QFrame()
        self.chat_settings_panel.setObjectName("subsection")
        settings_layout = QVBoxLayout(self.chat_settings_panel)
        self.chat_parameters = QLabel("有效参数：等待模型状态…")
        self.chat_parameters.setObjectName("sectionHint")
        self.chat_parameters.setWordWrap(True)
        settings_layout.addWidget(self.chat_parameters)
        ...
        outer.addWidget(self.chat_settings_panel)
    ...
    def _toggle_chat_settings(self):
        expanded = not self.chat_settings_panel.isVisible()
        self.chat_settings_panel.setVisible(expanded)
        self.chat_settings_button.setText("收起设置" if expanded else "对话设置")
    ...
    def _network_page(self):
        ...
        save_api_row = QHBoxLayout()
        save_api_row.addWidget(button("保存接口设置", self.save_network, "primaryButton"))
        save_api_row.addStretch()
        api_layout.addLayout(save_api_row)
    ...
    def _update_buttons(self):
        ...
        self.cancel_btn.setVisible(self.chat is not None)
